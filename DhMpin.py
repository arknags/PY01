...
def main():
    ...
    alerts = []
    send_telegram_alert("***** 6. MPin started *********", TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
    ...
    for symbol, sdf in df.groupby("symbol"):
        ...
        if not monthly_df.empty and len(monthly_df) > 13 and is_equity:
            
            # Volume Spike Detection
            recent = monthly_df.tail(13)
            today_volume = recent.iloc[-1]["volume"]
            avg_past_12 = recent.iloc[:-1]["volume"].mean()
            vol_spike = today_volume > avg_past_12   # you can adjust the multiplier
            ...
            if vol_spike :        
                ...
                #     Msg = f"NO Monthly Pin data for {symbol}"
                #     send_telegram_alert(Msg, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)

    print(f"✅ Alerts sent for: {alerts}")
    if alerts:
        send_telegram_alert("***  Monthly Pin Scan Completed ***", TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
    else:
        send_telegram_alert("***  No monthly Pin Bar Signals **", TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)

    subprocess.run(["/Users/nags/Data/Alerts/colab/venv/bin/python", "/users/Nags/Data/Alerts/colab/DhWpin.py"])
