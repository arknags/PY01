...
def main():
    ...
    alerts = []
    send_telegram_alert("*** 10. Weekly Volume check scan started ***", TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
    ...
    for symbol, sdf in df.groupby(symbol_column):
        ...
        if not monthly_df.empty and len(monthly_df) > 20:
            recent = monthly_df.tail(20)
            today_volume = recent.iloc[-1]["volume"]
            avg_past_12 = recent["volume"].tail(20).mean()

            vol_spike = today_volume > avg_past_12  * 8  # You can apply a multiplier like avg_past_12 * 5

            if vol_spike:
                msg = (
                    f"📌 Weekly Volume spike detected:\n"
                    f"📅 {recent.iloc[-1]['date'].date()}, "
                    f"💹 {symbol}, "
                    f"📈 Close: {recent.iloc[-1]['close']}, "
                    f"🔊 Volume: {today_volume}, "
                    f"📊 20WK Avg Volume: {round(avg_past_12)}"
                )
    ...
    if alerts:
        send_telegram_alert("***  Weekly Volume Scan Completed ***", TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
    else:
        send_telegram_alert("***  No Weekly Volume spike Bar Signals ***", TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
