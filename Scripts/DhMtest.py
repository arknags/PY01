...
def check_supertrend_signals(row, level_name='Supertrend', proximity_pct=25):
    ...
    return {
        "MST_BUY Touch" : btouch,
        "MST_BUY bounce": bbounce,
        "MST_BUY proximity": bproximity
        # "MST_SELL rejection": srejection,  # optional
...
def main():
    ...
    alerts = []
    send_telegram_alert("***** 1. MST started *********", bot_token, chat_id)
    for symbol, sdf in df.groupby(symbol_column):
        ...
        if len(monthly_df) >= 2:
            monthly_df = compute_supertrend(monthly_df)
        
            latest = monthly_df.iloc[-1]
            signals = check_supertrend_signals(latest)
            #print("monthly DF is :" , monthly_df)
            #if signals and (signals["MST_BUY bounce"] or signals["MST_BUY proximity"]):
            if signals["MST_BUY Touch"]:
                msg = f"🔔 MST - {latest['date'].date()}, {symbol}, close {latest['close']}, ST = {latest['Supertrend']}\n"
                #msg = f"🔔 {symbol} (monthly)\nDate: {latest['date'].date()}\n"
                if signals["MST_BUY Touch"]:
                    msg += "✅ BUY Touch MONTHLY Supertrend"
                # elif signals["MST_BUY proximity"]:
                #     msg += "⚠️ BUY Proximity to Supertrend"

                send_telegram_alert(msg, bot_token, chat_id)
                alerts.append(symbol)
        else:
            print("Failed monthly df", monthly_df)
            print("monthly_df has insufficient rows for Supertrend calculation")
            Msg = f"NO MST data for {symbol}"
            send_telegram_alert(Msg, bot_token, chat_id)


    print(f"✅ Alerts sent for: {alerts}")
    if alerts:
        send_telegram_alert("*****  MST Completed ***********", bot_token, chat_id)
    else :
        send_telegram_alert("*****  No MST signals *********", bot_token, chat_id)

    subprocess.run(["/Users/nags/Data/Alerts/colab/venv/bin/python", "/users/Nags/Data/Alerts/colab/DhWtest.py"])
