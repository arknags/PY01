...
import subprocess 
import json
# provide alert when price touches or bounces from Support pivot resistance levels.
#13th July 2025 - Modify the script to alert when price crosses with volume above support, pivot, resistance levels
...
def check_cpr_signals(row, level_name, proximity_pct=0.1):
    ...
    return {
        ...
        #f"{level_name}_SELL proximity": srejection
...
def main():
    ...
    alerts = []
    all_final_signals = []
    send_telegram_alert("-----4. MCPR started-----", bot_token, chat_id)
    for symbol, sdf in df.groupby(symbol_column):
        ...
        if len(monthly_df) < 2:
            continue  # skip if less data

        monthly_df['volMean20'] = monthly_df['volume'].rolling(window=10).mean()
        ...
        monthly_cpr_df["month"] = monthly_cpr_df["date"].dt.to_period("M")
        sdf["month"] = sdf.index.to_period("M")
        ...
        cpr_levels = ['pivot', 'tc', 'bc', 'r1', 's1', 's2']
        ...
        if signal_df.iloc[0].eq(False).all():
            print(f"❌ No Monthly CPR Signals for {symbol}")
            continue
        ...
        alert_message = f"🚨 {datetime.now().date()} Monthly CPR Signal for {symbol}, close = {last_row['close']:.2f}:\n"
    ...
    if all_final_signals:
        ...
        print("✅ Alerts saved to final_monthly_signals.xlsx")
        send_telegram_alert("-----MCPR check Completed-----", bot_token, chat_id)
    else:
        print("❌ No MCPR signals generated for any symbol")
        send_telegram_alert("NO MCPR signals", bot_token, chat_id)
    subprocess.run(["/Users/nags/Data/Alerts/colab/venv/bin/python", "/Users/nags/Data/Alerts/colab/DhQCPR2.py"])
