...
# ========== SIGNAL CHECK ==========
def check_supertrend_signals(row, level_name='Supertrend', proximity_pct=15, volSpike=False):
    level = row.get(level_name)
    atr = row.get('ATR')
    if pd.isna(level) or pd.isna(atr) or not volSpike:
        return None
    ...
    bbounce = (bproximity or btouch) and (close > level) and (
        ((lower_wick > 2.5 * body and upper_wick < 0.45 * body and body / range1 < 0.4) or
         (body > 0.6 * range1))
    )

    srejection = (sproximity or stouch) and (close < level) and (
        ((upper_wick > 2.5 * body and lower_wick < 0.45 * body and body / range1 < 0.4) or
         (body > 0.6 * range1))
    )

    return {
        "DST_BUY Touch": btouch,
        "DST_BUY bounce": bbounce,
        "DST_BUY proximity": bproximity
        # "DST_SELL rejection": srejection,
        # "DST_SELL proximity": sproximity
    }
...
def main():
    ...
    send_telegram_alert("***** 3. DST started *********", bot_token, chat_id)
    alerts = []
    for symbol, sdf in df.groupby(symbol_column):
        sdf['volMean20'] = sdf['volume'].rolling(window=20).mean()
        sdf['volSpike'] = sdf['volume'] >   sdf['volMean20']  

        if len(sdf) >= 2:
            sdf = compute_supertrend(sdf)
        else:
            print(f"{symbol} skipped: insufficient rows")
            continue

        latest = sdf.iloc[-1]

        # ✅ Check volume spike for latest candle only
        if not latest['volSpike']:
            continue

        signals = check_supertrend_signals(latest, volSpike=latest['volSpike'])
        #if signals and (signals["DST_BUY bounce"] or signals["DST_BUY proximity"]):
        if  (signals["DST_BUY Touch"]):
            msg = f"🔔 DST - {latest['date'].date()}, {symbol}, close {latest['close']}, ST = {latest['Supertrend']}\n"
            if signals["DST_BUY Touch"]:
                msg += "✅ BUY Touches DST"
            # elif signals["DST_BUY proximity"]:
    ...
    if alerts:
        send_telegram_alert("*****  DST Completed ***********", bot_token, chat_id)
    else:
        send_telegram_alert("*****  No DST signals *********", bot_token, chat_id)
    subprocess.run(["/Users/nags/Data/Alerts/colab/venv/bin/python", "/Users/nags/Data/Alerts/colab/DhMCPR2.py"])
